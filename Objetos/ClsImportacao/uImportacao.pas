...
// Configura uma conex„o com o banco de dados especÌfico do portal
    ...
    begin
        try
             try
                 ...
                 // Atribui a hora de inÌcio do processo
                 tTimeInicial := Now;

                 // Chama a funÁ„o para baixar o aquivo XML
                 Randomize;
                 ...
                 // Chama procedure para contar o total de imÛveis e fotos no XML
                 ...
                 // Chama a funÁ„o para executar a conex„o com o respectivo portal onde ser„o gravados os dados
                 Self.ConectarPortal;
                 ...
                 // Inseri os dados para atualizaÁ„o da tarefa
                 self.AtualizaInfoTarefa(FIdAgendameto, Self.CalculaDuracao(tTimeInicial, Now), 'E', 'Comparando registros de imÛveis para inclus„o', '');

                 // Filtra todos os registros que n„o existem na tabela imÛveis2
                 oQryTemp.Close;
                 oQryTemp.SQL.Clear;
                 ...
                 begin
                     ...
                     begin
                          ...
                          // Inseri os dados para atualizaÁ„o da tarefa
                          self.AtualizaInfoTarefa(FIdAgendameto, Self.CalculaDuracao(tTimeInicial, Now), 'E', 'Executando inclus„o de novos imÛveis - ' + IntToStr(iContInserido) + ' de ' + IntToStr(iContTotalItem) , '');
                 ...
                 // Inseri os dados para finalizaÁ„o da tarefa
                 self.AtualizaInfoTarefa(FIdAgendameto, Self.CalculaDuracao(tTimeInicial, Now), 'E', 'Comparando registro de imÛveis para atualizaÁ„o', '');

                 // Filtra todos os registros que s„o diferentes da tabela imÛveis2
                 dm.cdsAux.Close;
                 dm.qryAux.Close;
                 ...
                 begin
                      ...
                      begin
                          ...
                          // Inseri os dados para finalizaÁ„o da tarefa
                          self.AtualizaInfoTarefa(FIdAgendameto, Self.CalculaDuracao(tTimeInicial, Now), 'E', 'Executando atualizaÁ„o de imÛveis - ' + IntToStr(iContAtualizado) + ' de ' + IntToStr(iContTotalItem) , '');
                 ...
                 // Inseri os dados para finalizaÁ„o da tarefa
                 self.AtualizaInfoTarefa(FIdAgendameto, Self.CalculaDuracao(tTimeInicial, Now), 'E', 'Comparando registro de imÛveis para exclus„o', '');

                 // Filtra todos os registros que n„o existem na tabela imÛveis_xml
                 oQryTemp.Close;
                 oQryTemp.SQL.Clear;
                 ...
                 begin
                      ...
                      begin
                          ...
                          // Inseri os dados para finalizaÁ„o da tarefa
                          self.AtualizaInfoTarefa(FIdAgendameto, Self.CalculaDuracao(tTimeInicial, Now), 'E', 'Executando exclus„o de imÛveis - ' + IntToStr(iContExcluido) + ' de ' + IntToStr(iContTotalItem) , '');
                 ...
                 // Inseri os dados para atualizaÁ„o da tarefa
                 self.AtualizaInfoTarefa(FIdAgendameto, Self.CalculaDuracao(tTimeInicial, Now), 'E', 'Limpando tabela tempor·ria de imÛveis', '');
...
begin
  {def map = ["√°":"·","√Å":"¡","√¢":"‚","√Ç":"¬","√ ":"‡","√Ä":"¿",
    ...
    texto := StringReplace(texto, '√ ', '‡', [rfReplaceAll]);
    texto := StringReplace(texto, '√£', '„', [rfReplaceAll]);
    texto := StringReplace(texto, '√©', 'È', [rfReplaceAll]);
    texto := StringReplace(texto, '√™', 'Í', [rfReplaceAll]);
    texto := StringReplace(texto, '√≠', 'Ì', [rfReplaceAll]);
    texto := StringReplace(texto, '√¥', 'Ù', [rfReplaceAll]);
    texto := StringReplace(texto, '√µ', 'ı', [rfReplaceAll]);
    texto := StringReplace(texto, '√≥', 'Û', [rfReplaceAll]);
    texto := StringReplace(texto, '√∫', '˙', [rfReplaceAll]);
    texto := StringReplace(texto, '√ß', 'Á', [rfReplaceAll]);
...
// FunÁ„o que c·lcula e retorna o tempo de duraÁ„o da importaÁ„o
...
// FunÁ„o para baixar o arqivo XML do cliente
...
// MÈtodo para carregar os dados de conex„o com o banco de dados do portal
     ...
     try
          ...
          try
               ...
               begin
                   ...
                   // Atualiza o contador com total de fotos depois da comparaÁ„o EXCEPT
                   ...
                   begin
                        ...
                        begin
                            ...
                            begin
                                // Redimensiona e grava novamente as imagens no diretÛrio atual
               ...
               // Filtra todos os registros que n„o existem na tabela imÛveis_fotos_xml
               oQryTemp.Close;
               oQryTemp.SQL.Clear;
               ...
               begin
                    ...
                    begin
                        ...
                        // Inseri os dados para finalizaÁ„o da tarefa
                        self.AtualizaInfoTarefa(FIdAgendameto, Self.CalculaDuracao(tTimeInicial, Now), 'E', 'Imagens excluÌdas ('+IntToStr(iContFotoExcluida)+')', '');
                        oQryTemp.Next;
                    end;
                    Self.FConexaoPortal.Close;
               end;
